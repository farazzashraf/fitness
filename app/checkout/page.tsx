...
function CheckoutContent() {
  ...
  const raw = searchParams.get('plan') || 'elite'
  const [planKey, setPlanKey] = useState<keyof typeof plans>(raw in plans ? (raw as keyof typeof plans) : 'elite')
  ...
  if (step === 'success') return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ textAlign: 'center', maxWidth: 480 }}>
        <div style={{ width: 80, height: 80, borderRadius: '50%', margin: '0 auto 28px', background: 'rgba(201,255,0,0.1)', border: '2px solid var(--accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 36, animation: 'popIn 0.4s ease' }}>✓</div>
        <h1 className="font-display" style={{ fontSize: 56, letterSpacing: 2, marginBottom: 12 }}>YOU&apos;RE IN!</h1>
        <p style={{ color: '#666', fontSize: 16, lineHeight: 1.7, marginBottom: 12 }}>
          Welcome to APEX <span style={{ color: plan.color, fontWeight: 700 }}>{plan.name}</span>. Your 14-day free trial has started.
        </p>
        <p style={{ color: '#444', fontSize: 14, marginBottom: 40 }}>A confirmation has been sent to your email.</p>
        <Link href="/dashboard" style={{ background: 'var(--accent)', color: '#000', padding: '14px 36px', borderRadius: 12, textDecoration: 'none', fontSize: 14, fontWeight: 800, letterSpacing: 2, textTransform: 'uppercase' }}>Go to Dashboard →</Link>
      </div>
  ...
  if (step === 'processing') return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ textAlign: 'center' }}>
        <div style={{ width: 64, height: 64, border: '3px solid #1a1a1a', borderTopColor: 'var(--accent)', borderRadius: '50%', margin: '0 auto 24px', animation: 'spin 0.8s linear infinite' }} />
  ...
  const inputStyle = (err?: string): React.CSSProperties => ({
    ...
    border: `1px solid ${err ? 'rgba(239,68,68,0.5)' : '#222'}`,
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', color: '#fff' }}>
      {/* Nav */}
      <nav style={{ borderBottom: '1px solid #1a1a1a', padding: '0 5%', background: 'rgba(8,8,8,0.97)', backdropFilter: 'blur(12px)', position: 'sticky', top: 0, zIndex: 50 }}>
      ...
      <div className="checkout-layout" style={{ maxWidth: 1000, margin: '0 auto', padding: '48px 5%', display: 'grid', gridTemplateColumns: 'minmax(0,1fr) clamp(280px, 30vw, 380px)', gap: 40 }}>
        ...
        <div>
          ...
          <div style={{ marginBottom: 32 }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: '#666', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 12 }}>Billing Period</div>
            <div style={{ display: 'flex', gap: 10 }}>
              {(['monthly', 'annual'] as const).map(b => (
                <button key={b} onClick={() => setBilling(b)} style={{ flex: 1, padding: '13px 0', background: billing === b ? 'rgba(201,255,0,0.08)' : '#0f0f0f', border: `1px solid ${billing === b ? 'rgba(201,255,0,0.35)' : '#222'}`, borderRadius: 12, color: billing === b ? 'var(--accent)' : '#555', fontSize: 13, fontWeight: 700, cursor: 'pointer', transition: 'all 0.2s' }}>
                  {b === 'annual' ? <span>Annual <span style={{ background: 'rgba(201,255,0,0.15)', color: 'var(--accent)', fontSize: 10, padding: '2px 7px', borderRadius: 100, marginLeft: 6, fontWeight: 800 }}>SAVE 20%</span></span> : 'Monthly'}
                </button>
              ))}
            </div>
          </div>
          ...
          <div style={{ background: '#0f0f0f', border: '1px solid #1a1a1a', borderRadius: 20, padding: 28, marginBottom: 20 }}>
            <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 24, display: 'flex', alignItems: 'center', gap: 8 }}>💳 Card Details</div>

            <div style={{ marginBottom: 16 }}>
              <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#666', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 8 }}>Cardholder Name</label>
              <input value={cardName} onChange={e => setCardName(e.target.value)} placeholder="Name on card" style={inputStyle(errors.cardName)}
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#666', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 8 }}>Card Number</label>
              <div style={{ position: 'relative' }}>
                <input value={cardNumber} onChange={e => setCardNumber(formatCard(e.target.value))} placeholder="1234 5678 9012 3456"
                  style={{ ...inputStyle(errors.cardNumber), padding: '13px 52px 13px 16px', letterSpacing: 2 }}
                  onFocus={e => (e.target.style.borderColor = 'rgba(201,255,0,0.4)')}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <div>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#666', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 8 }}>Expiry Date</label>
              ...
              <div>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#666', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 8 }}>CVV</label>
                <input value={cvv} onChange={e => setCvv(e.target.value.replace(/\D/g, '').slice(0, 4))} placeholder="•••" style={inputStyle(errors.cvv)}
                  onFocus={e => (e.target.style.borderColor = 'rgba(201,255,0,0.4)')}
          ...
          <div style={{ background: '#0f0f0f', border: '1px solid #1a1a1a', borderRadius: 20, padding: '20px 24px', marginBottom: 28 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <div style={{ border: '1px solid #333', borderRadius: 8, padding: '6px 14px', fontSize: 12, color: '#555', cursor: 'pointer' }}>Use Instead</div>
            </div>
          </div>

          <button onClick={handlePay} style={{ width: '100%', background: 'var(--accent)', color: '#000', border: 'none', borderRadius: 14, padding: '17px 0', fontSize: 15, fontWeight: 800, letterSpacing: 2, textTransform: 'uppercase', cursor: 'pointer', transition: 'all 0.2s' }}
            onMouseEnter={e => e.currentTarget.style.boxShadow = '0 8px 32px rgba(201,255,0,0.35)'}
            onMouseLeave={e => e.currentTarget.style.boxShadow = 'none'}
          >🔒 Start Free Trial -Pay Nothing Today</button>
        ...
        <div>
          <div style={{ background: '#0f0f0f', border: '1px solid #1a1a1a', borderRadius: 20, padding: 28, position: 'sticky', top: 88 }}>
            ...
            {/* Plan switcher - 2 columns */}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 24 }}>
              {(Object.keys(plans) as Array<keyof typeof plans>).map(k => (
                <button key={k} onClick={() => setPlanKey(k)} style={{
                  padding: '14px 0',
                  background: planKey === k ? 'rgba(201,255,0,0.08)' : '#141414',
                  border: `1px solid ${planKey === k ? plans[k].color : '#222'}`,
                  borderRadius: 12, cursor: 'pointer', transition: 'all 0.2s',
                  display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4,
                }}>
                  <span style={{ fontSize: 13, fontWeight: 800, color: planKey === k ? plans[k].color : '#666' }}>{plans[k].name}</span>
                  <span style={{ fontSize: 11, color: planKey === k ? '#aaa' : '#444' }}>
            ...
            {/* Gym tag */}
            <div style={{ background: 'rgba(201,255,0,0.04)', border: '1px solid rgba(201,255,0,0.12)', borderRadius: 10, padding: '10px 14px', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ fontSize: 18 }}>🏋️</span>
              <span style={{ fontSize: 12, fontWeight: 600, color: '#888' }}>{plan.gymTag}</span>
            </div>
            ...
            <div style={{ borderTop: '1px solid #1a1a1a', paddingTop: 20 }}>
              <div style={{ background: 'rgba(201,255,0,0.05)', border: '1px solid rgba(201,255,0,0.15)', borderRadius: 10, padding: '12px 14px', marginBottom: 16, fontSize: 12, color: '#888', lineHeight: 1.6 }}>
                🎁 14-day free trial. First charge after trial ends.
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 16 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: '#666' }}>
                  <span>Subtotal</span><span>₹{price.toLocaleString('en-IN')}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: '#666' }}>
                  <span>GST (18%)</span><span>₹{Math.round(price * 0.18).toLocaleString('en-IN')}</span>
                </div>
                {billing === 'annual' && (
              ...
              <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid #222', borderRadius: 8, padding: '12px 16px', marginTop: 16 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: 13, color: '#ccc', fontWeight: 600 }}>After 14-day trial</span>
            ...
            <div style={{ display: 'flex', gap: 12, marginTop: 24, justifyContent: 'center' }}>
              {['🔒 Secure', '↩ Cancel anytime', '🏋️ Gym included'].map(b => (
