...
// ─── Responsive Aside ─────────────────────────────────────────────────────────
function AsideSidebar({ isOpen, onClose, children }: { isOpen?: boolean; onClose?: () => void; children: React.ReactNode }) {
...
function Sidebar({ active, setActive, isOpen, onClose }: {
  active: Tab; setActive: (t: Tab) => void; isOpen?: boolean; onClose?: () => void
}) {
  ...
  return (
    <AsideSidebar isOpen={isOpen} onClose={onClose}>
      {/* Logo + close btn */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 8px', marginBottom: 40 }}>
        ...
        {/* {onClose && (
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: '#555', cursor: 'pointer', fontSize: 20, padding: '4px 6px', lineHeight: 1 }}>✕</button>
        )} */}
      </div>
      ...
      <div style={{ borderTop: '1px solid #111', paddingTop: 20, marginTop: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px' }}>
          <div style={{ width: 34, height: 34, borderRadius: '50%', background: 'linear-gradient(135deg, var(--accent), #666)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 800, color: '#000', flexShrink: 0 }}>AD</div>
          <div>
...
function OverviewTab() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      {/* Stat grid — auto-fit so it wraps at any width */}
      ...
      {/* Plan dist + Attendance — stacks at 900px via CSS class */}
...
function AnalyticsTab() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      {/* Traffic sources + top pages — stack at 900px */}
      <div className="admin-grid-2col" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        <div style={{ background: '#0f0f0f', border: '1px solid #1a1a1a', borderRadius: 16, padding: '24px 20px' }}>
...
function ChurnTab() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      {/* At-risk — each row stacks on mobile */}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {atRiskReasons.map(r => (
            <div key={r.customer} style={{ background: '#0a0a0a', border: '1px solid #1a1a1a', borderRadius: 12, padding: '16px 18px' }}>
              {/* Always column layout — works on all sizes */}
...
export default function AdminDashboard() {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#080808', color: '#fff' }}>
      ...
      <Sidebar active={activeTab} setActive={setActiveTab} isOpen={sidebarOpen} onClose={() => setSidebarOpen(false)} />

      <main className="dash-main" style={{ flex: 1, padding: '32px 36px', overflowY: 'auto', minWidth: 0 }}>

        {/* ── Mobile top bar (hidden on desktop via CSS) ── */}
        <div className="mobile-topbar" style={{ display: 'none', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, paddingBottom: 16, borderBottom: '1px solid #1a1a1a' }}>
          ...
          <div style={{ fontSize: 10, color: '#C9FF00', fontWeight: 700, letterSpacing: 1 }}>ADMIN</div>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28, flexWrap: 'wrap', gap: 10 }}>
          ...
          {/* Hide badges below 900px via CSS class */}
