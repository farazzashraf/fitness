...
export default function LoginPage() {
  ...
  const FAKE_USER_EMAIL = process.env.NEXT_PUBLIC_FAKE_USER_EMAIL
  const FAKE_USER_PASSWORD = process.env.NEXT_PUBLIC_FAKE_USER_PASSWORD
  ...
  const handleSubmit = async () => {
    ...
    // 2. Logic: Check Admin first, then check regular User
    if (inputEmail === ADMIN_EMAIL && password === ADMIN_PASSWORD) {
        router.push('/admin')
    } else if (inputEmail === FAKE_USER_EMAIL && password === FAKE_USER_PASSWORD) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: 'radial-gradient(circle, rgba(201,255,0,0.07) 0%, transparent 70%)',
        pointerEvents: 'none',
      }} />
      <div style={{
      ...
      <div style={{ width: '100%', maxWidth: 440, position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ background: '#0f0f0f', border: '1px solid #1e1e1e', borderRadius: 24, padding: '40px 36px' }}>
          ...
          <p style={{ color: '#555', fontSize: 14, textAlign: 'center', marginBottom: 32 }}>
          ...
          <div style={{ textAlign: 'right', marginBottom: 28 }}>
            <span style={{ fontSize: 13, color: '#555', cursor: 'pointer' }}
          ...
          <button
            ...
            style={{
              width: '100%', background: loading ? '#1a1a1a' : 'var(--accent)',
              color: loading ? '#555' : '#000', border: 'none', borderRadius: 12,
              padding: '15px 0', fontSize: 14, fontWeight: 800, letterSpacing: 2,
